...
![Version: 2.0.1](https://img.shields.io/badge/Version-2.0.1-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 5.7.2-1-ce](https://img.shields.io/badge/AppVersion-5.7.2--1--ce-informational?style=flat-square)
...
| app.image.tag | string | `"5.7.2-1-ce"` | Overrides the image tag whose default is the chart appVersion. |
...
| mariadb.image.repository | string | `"bitnamilegacy/mariadb"` | Pin redis image to bitnamilegacy image repository. |
...
| passboltEnv.plain.CACHE_CAKECORE_CLASSNAME | string | `"Redis"` | Configure passbolt cake core cache class |
| passboltEnv.plain.CACHE_CAKECORE_HOST | string | `"127.0.0.1"` | Configure passbolt cake core cache server |
| passboltEnv.plain.CACHE_CAKECORE_PORT | int | `6379` | Configure passbolt cake core cache server port |
| passboltEnv.plain.CACHE_CAKEMODEL_CLASSNAME | string | `"Redis"` | Configure passbolt cake model cache class |
| passboltEnv.plain.CACHE_CAKEMODEL_HOST | string | `"127.0.0.1"` | Configure passbolt cake model cache server |
| passboltEnv.plain.CACHE_CAKEMODEL_PORT | int | `6379` | Configure passbolt cake model cache server port |
| passboltEnv.plain.CACHE_DEFAULT_CLASSNAME | string | `"Redis"` | Configure passbolt cake cache class |
| passboltEnv.plain.CACHE_DEFAULT_HOST | string | `"127.0.0.1"` | Configure passbolt cake cache server |
| passboltEnv.plain.CACHE_DEFAULT_PORT | int | `6379` | Configure passbolt cake cache server port |
...
| passboltEnv.plain.PASSBOLT_SECURITY_DISPLAY_NON_WEBUSER_WARNING | bool | `true` | Set to false to supress warnings when running passbolt commands as non webserver user, mainly for Openshift |
...
| passboltEnv.plain.SESSION_DEFAULTS | string | `"cache"` | Configure passbolt cake session storage, either php, cake, database or cache |
| passboltEnv.secret.CACHE_CAKECORE_PASSWORD | string | `"CHANGEME"` | Configure passbolt cake core cache password |
| passboltEnv.secret.CACHE_CAKEMODEL_PASSWORD | string | `"CHANGEME"` | Configure passbolt cake model cache password |
| passboltEnv.secret.CACHE_DEFAULT_PASSWORD | string | `"CHANGEME"` | Configure passbolt cake cache password |
...
```bash
helm-docs -t .assets/readme/README_header.md.gotmpl -t .assets/readme/README.md.gotmpl --dry-run | mdformat - > README.md && helm-docs -t .assets/readme/app_readme_header.md.gotmpl -t .assets/readme/README.md.gotmpl --dry-run | mdformat - > app-readme.md
```
